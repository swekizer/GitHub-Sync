...
* **Token Storage**: Your Personal Access Token is stored **unencrypted** in the plugin's `data.json` file within your vault's `.obsidian` folder. This is standard for Obsidian plugins, but be mindful if you share your vault folder, back it up to a public location, or use a shared device. Consider using a PAT with an expiration date and only the minimum required scopes (`repo`).
